		...
		function ModelsCard(props) {
			...
			const testConnection = async () => {
				...
				try {
					...
					if (res.status === 0 || !json || json.ok !== true) {
						setConnLine({ ok: false, text: t("test.failed", { msg: String((json && json.error) || "HTTP " + res.status) }) });
						return;
					}
					// the probe reports one line per wire protocol it checked,
					// because the two carry different credentials
					...
					const summary = checks.map((check) => apiLabel(check.api) + " ✓").join(" · ");
					setConnLine({
						ok: true,
