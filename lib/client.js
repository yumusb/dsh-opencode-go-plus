...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"models.enabledList": "已启用 {n} 个（点「获取可用模型」可增删）：",
			"models.via": "候选来源：{url}",
		...
		const en = {
			...
			"models.enabledList": "{n} enabled (use Fetch available models to change the set):",
		...
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"" + CSS_ID + "\"]") === null) {
			...
			tag.textContent = [
				...
				".ocgp-cand-static{cursor:default}",
				".ocgp-cand-static:hover{background:none}",
		...
		function ModelsCard(props) {
			...
			//
			// It has to be a write, not a draft edit. `picked` is empty until the
			// candidate list is fetched, so editing it removed nothing while the
			// card reported success and pointed at an "Enable selected" button
			// that is not rendered without that list. The committed set is taken
			// from the served ids the check just iterated, which also covers a
			// model that is enabled but no longer in the gateway's listing and so
			// could never have been a candidate.
			//
			// When the candidate list IS loaded the user may have unapplied edits
			// in the checkboxes; writing then would discard them silently, so that
			// case asks them to commit or discard first.
			const dropUnavailable = async () => {
				const gone = [...unavailable];
				if (gone.length === 0) return;
				if (candidates !== null && dirty) {
			...
			// Rows exist in two states. Before a fetch the card shows what is
			// SERVED, read-only, straight from models-info — so opening it answers
			// "which models are enabled?" without a round trip. After a fetch it
			// shows the editable candidate list. Both render through the same row
			// code, so a model looks the same either way.
			const servedRows = info && Array.isArray(info.entries) ? info.entries : [];
			const editing = candidates !== null;
			...
			const visible = candidates === null ? servedRows : candidates.filter((c) => {
				...
				children: [
					...
					react_jsx_runtime.jsx("div", {
						...
						children: [
							...
							// Name the served list so it is clear this is the current
							// state, not a selectable list awaiting a fetch.
							!editing && servedRows.length > 0 ? react_jsx_runtime.jsx("div", {
								className: "ocgp-line",
								children: t("models.enabledList", { n: servedRows.length })
							}) : null,
							...
							visible.length > 0
								? react_jsx_runtime.jsx("ul", {
									...
									children: visible.map((c, index) => {
										...
										/** The model's own facts; shared by both modes. */
										const body = react_jsx_runtime.jsx("div", {
											...
											children: [
												react_jsx_runtime.jsx("div", {
													...
													children: [
														c.id,
														// in the served view everything listed IS
														// enabled, so the tag would be noise
														editing && c.enabled ? react_jsx_runtime.jsx("span", {
															className: "ocgp-tag",
										...
										// A row is a checkbox label only while editing;
										// otherwise it is plain text, because there is
										...
										return react_jsx_runtime.jsx("li", {
											children: editing
												...
												: react_jsx_runtime.jsx("div", { className: "ocgp-cand ocgp-cand-static", children: body })
										}, c.id);
									})
								})
								: null,
