...
export function classifyPiAiError(message) {
	// The OpenCode gateway rejects opt-in models (e.g. muse-*) with 403
	// DataPolicyError until the account accepts their data-use terms at the
	// URL in the message. Classifying that as AUTH renders "invalid API key",
	// which sends the user hunting for a credential problem that does not
	// exist — so it gets its own code, and the raw gateway message (which
	// carries the opt-in URL) is surfaced verbatim.
