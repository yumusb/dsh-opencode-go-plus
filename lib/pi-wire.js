 ...
 * requires and the DSH block does not carry.
 * @param {object} input
 ...
 * @returns {any}
 */
export function toPiContext({ options, images }) {
	/** @type {any[]} */
	const messages = [];
	...
	for (const message of options.messages) {
		...
		for (const block of message.content) {
			...
			// reasoning blocks are model-internal and are not replayed
		}
		if (parts.length === 0) continue;
		messages.push({
			role: "assistant",
			content: parts,
			api: "openai-completions", // replaced by pi-ai when replaying its own output
			provider: "dsh-opencode-go-plus",
			model: options.model,
...
function mapStopReason(message, contextWindow) {
	const overflow = /context|too long|too large/i.test(String(message?.errorMessage ?? ""))
		|| isContextWindowExceededError(String(message?.errorMessage ?? ""));
	if (message?.stopReason === "error" && overflow) {
		return {
			kind: "error",
			failure: { message: message.errorMessage ?? "context window exceeded", code: CONTEXT_WINDOW_EXCEEDED_CODE }
		};
	}
	switch (message?.stopReason) {
		...
		case "aborted": return { kind: "error", failure: { message: message.errorMessage ?? "aborted", code: "ABORTED" } };
		case "pending": return { kind: "error", failure: { message: "stream ended pending", code: "PI_AI_ERROR" } };
		default: return { kind: "error", failure: { message: message?.errorMessage ?? `stopped: ${String(message?.stopReason)}`, code: "PI_AI_ERROR" } };
	}
	// `contextWindow` is retained for callers that want it when classifying
	void contextWindow;
}
