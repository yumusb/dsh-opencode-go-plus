	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (_req, res) => {
			...
			try {
				...
				// The full rows, not just ids, so the card can list what is
				// actually served the moment it opens. Reading only the ids left
				// it claiming "7 models enabled" with no list under it until the
				// user fetched candidates — a fetch that exists to EDIT the
				// selection, and should not be required just to see it.
				const entries = served.map((entry) => modelRowFields(entry.id, entry, liveCatalog, documented));
