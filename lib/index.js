	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (_req, res) => {
			...
			try {
				...
				// listed by /v1/models long after the provider retired it. This
				// costs no request, so it is worth saying on every card load.
				const documented = new Set(catalogIds(liveCatalog));
	...
	ctx.effect(() => ctx.webServer.register({
		...
		handler: async (_req, res) => {
			...
			try {
				...
				const metadata = await refreshLiveCatalog();
				// A model can be live on the gateway but absent from the catalog
				// (union-alpha was): the catalog then says nothing about its
				// protocol or capabilities, which used to render a blank
				// candidate with no protocol at all. Fall back to what the stored
				// entry already records, and mark it so the card can distinguish
				// "not documented" from "documented like this".
				...
				const candidates = liveIds.map((id) => {
					...
					return {
						...fields,
						...(documented.has(id) ? {} : { ...stored.get(id), id, undocumented: true }),
