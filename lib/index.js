...
async function probeCredential({ baseURL, apiKey, api, model, timeoutMs }) {
	...
	try {
		...
		return {
			...
			api,
			model,
			detail: `无法连接 ${url}:${error instanceof Error ? error.message : String(error)}`
		};
	}
	...
	if (res.ok) return { ok: true, status: res.status, api, model, detail: "凭据有效" };
	// anything the gateway refused for a reason OTHER than auth means the key passed
	const authFailure = res.status === 401 || res.status === 403 || /invalid api key|autherror|unauthor/i.test(message);
	if (authFailure) return { ok: false, status: res.status, api, model, detail: message };
	if (res.status === 400) return { ok: true, status: res.status, api, model, detail: "凭据有效（请求体被拒，这是预期的）" };
	return { ok: false, status: res.status, api, model, detail: message };
}
