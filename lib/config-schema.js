...
export const DEFAULTS = Object.freeze({
	...
	timeoutMs: 15000,
	/**
	 * Per-request timeout for a CHAT call, distinct from `timeoutMs` (metadata
	 * and listing calls).
	 *
	 * This MUST be set: without it the Anthropic SDK falls back to its own
	 * 10-minute default, so a stalled relay hangs the turn for ten minutes and
	 * only then fails. Five minutes still allows a long reasoning turn while
	 * bounding the hang.
	 */
...
// DSH 0.1.7's Settings form scanner accepts any Config with toJSON(), then
// reads the Schemastery-facing `meta` and `type` properties before deciding
// whether it has volatile fields. This dependency-free Standard Schema is not
// a Schemastery instance, so expose the inert shape the scanner needs. The
// custom plugin card remains the editor; no field is declared volatile, which
// also preserves the plain values expected by DSH 0.1.6.
Config.meta = Object.freeze({});
Config.type = "custom";
...
Config.toJSON = () => ({
	...
	refs: {
		...
		11: { type: "number", meta: { default: DEFAULTS.streamTimeoutMs } },
		...
		10: {
			type: "object",
			meta: { default: {} },
			dict: { providerRoute: 1, baseURL: 2, apiKeyEnv: 3, retries: 4, timeoutMs: 5, defaultContextWindow: 6, defaultMaxTokens: 7, models: 8, modelSource: 9, streamTimeoutMs: 11 }
		}
	}
});
