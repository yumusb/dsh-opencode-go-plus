...
| 已启用的模型 | 打开卡片就列出来，只读，带每个模型的上下文、输出上限和模态。不花请求：和卡片其余部分同一份数据。 |
| 获取可用模型 | 把列表切成可编辑状态：拉网关的实时模型当候选，已启用的自动勾上。 |
...
两种情况卡片都会给一个「移除这些模型并保存」，点下去立刻提交，不用先获取模型列表 —— 这个按钮的意义就在于此。唯一的例外是勾选框里还有未应用的改动：这时它会让你先应用或放弃，而不是默默覆盖掉。
