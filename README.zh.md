...
| 模型列表 | 打开卡片就加载好网关的实时模型，直接可编辑，不用先点按钮。已启用的自动勾上，每行带上下文、输出上限和模态。 |
| 重新获取 | 重新读一次网关列表，比如上游新增了模型之后。 |
| 检查模型可用性 | 对**每一个已启用的模型**各发一次请求，报出提供方已经不再提供的那些，另外把被政策或地区限制的单独列出来。探测请求会在生成前被拒，不消耗 token。 |
| 搜索 / 筛选 | 按 id 或名字找；按 全部 / 已启用 / 未启用 和“多模态”组合筛选。列表可编辑后才出现。 |
...
`/v1/models` 只是个广告位，不是事实来源 —— 提供方早就撤掉的模型，它照样列着。`union-alpha` 在被撤之后还一直挂在列表里，直到每次调用都开始报 `Model is unavailable.`。所以本插件会分两件事告诉你：

- **目录已不再收录，可能已下线** —— 打开卡片就能看到，不花任何请求。目录是元数据的来源，它撤掉的模型，协议和能力都再也刷新不了了。但这只是线索，不是证据：新模型总是先在网关上出现，目录过一阵才跟上。
- **提供方已不再提供** —— 点「检查模型可用性」之后才给，这是实打实的结论。它还会把「已下线」和「被政策或地区限制」分开，因为这两种要采取的办法完全不同。

两种情况卡片都会给一个「移除这些模型并保存」，点下去立刻提交。唯一的例外是列表里还有未应用的改动：这时它会让你先应用或放弃，而不是默默覆盖掉。
...
最省事的做法是完全不搬模型字段：在卡片上填好网关地址和 key，在它加载出来的列表里勾上要的就行。名字、容量、模态、协议都会从目录补全。
...
- **已启用的模型突然不能用了（报 `Model is unavailable.`）。** 见「已经下线的模型」那一节。网关会一直挂着已经撤掉的模型，所以启用着的模型可能悄悄烂掉；点「检查模型可用性」会把它们列出来，卡片上可以一键移除。
